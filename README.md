...
| Kafka Java type headers      | `spring.json.add.type.headers: true` - embeds the full Java class name in every message        | Set to `false` and use `spring.json.value.default.type` on consumers instead. Type headers couple consumers to the producer's package structure; disabling them produces portable plain JSON that any language can consume |
